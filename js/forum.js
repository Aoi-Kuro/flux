...
const FORUM_LAST_SEEN_KEY   = 'phys162_forum_last_seen_id';
...
const FORUM_DEVICE_ID_KEY = 'phys162_forum_device_id';
const FORUM_NAME_KEY      = 'phys162_forum_display_name';
const FORUM_NICKNAME_KEY  = 'phys162_forum_nickname'; // set only once a nickname is claimed
const FORUM_PIN_KEY       = 'phys162_forum_pin';       // the 5-digit PIN for that nickname
...
async function callForumClaimNickname(nickname, pin) {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/claim-nickname`, {
    ...
    body: JSON.stringify({ device_id: getForumDeviceId(), nickname, pin: pin || undefined }),
  });
  const data = await res.json().catch(() => null);
...
// Quiz options are built from QUIZZES (js/quizzes.js) so a new quiz added
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/post-message`, {
      ...
      body: JSON.stringify({
        ...
        reply_to_id: forumReplyTarget ? forumReplyTarget.id : null,
      }),
    });
    ...
    // Refresh the currently-viewed feed so the new message appears right away
    // if it matches the active filter, and re-sync "last seen" so this
    // message you just posted (and are looking at) doesn't get counted as
    // unread if you head back to the main screen a moment later.
...
async function submitForumExitDevice() {
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/drop-nickname`, {
      ...
      body: JSON.stringify({ device_id: getForumDeviceId(), action: 'exit' }),
    });
    const data = await res.json().catch(() => null);
...
async function submitForumEditMessage() {
  ...
  const payload = { device_id: getForumDeviceId(), message_id: forumEditMessageId };
...
const FORUM_AVATAR_CACHE_KEY = 'phys162_forum_avatar_cache';
...
function renderForumMessageBody(bodyEl, text) {
  bodyEl.textContent = '';
  ...
  lines.forEach((line) => {
    ...
    const taggedChips = renderForumLineWithMentions(lineEl, line, ownName);
...
async function flagForumMessage(messageId, btnEl) {
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/flag-message`, {
      ...
      body: JSON.stringify({ message_id: messageId, device_id: getForumDeviceId() }),
    });
    const data = await res.json().catch(() => null);
...
function renderForumMessage(msg) {
...
const FORUM_PINNED_MESSAGE_ID  = 336;
const FORUM_PIN_DISMISSED_KEY  = 'phys162_forum_pin_dismissed';
const FORUM_PIN_FIRST_SEEN_KEY = 'phys162_forum_pin_first_seen';
